...
logger = logging.getLogger("ai_services.prakriti")
...
Return strictly as JSON with this exact structure, where each recommendation is a single, clear string:
{{"recommendations": ["Eat warm, cooked meals with healthy fats like ghee.", "Practice a regular daily routine.", ...]}}"""
    ...
    try:
        ...
        for rec in recommendations_raw:
            if isinstance(rec, dict):
                # If LLM returns dicts instead of strings, join the values
                text = " - ".join(str(v) for v in rec.values() if v)
                if text:
        ...
        logger.warning("Recommendation generation failed: %s", str(e))
